  ...
  for(const [key, val] of ALEGROUPBYPASSER_SERVICE.group_collections) {
    if(!node.widgets || !node.widgets.find((w) => w.name === val.title)) {
      ...
      */
      ...
      node.inputs[node.inputs.length - 1].widget = JSON.stringify(boolWidget, (key, value) => key === '_node' ? undefined : value);
  ...
  if(node.widgets) {
    for(const widget of node.widgets) {
      if(widget._inputslot_origin_id) {
        const upstreamNode = app.graph.getNodeById(widget._inputslot_origin_id);
        ...
        nodeType.prototype.onConfigure = function (info) {
          ...
          for(let i=0;i<info.inputs.length;i++) {
            ...
            */
            ...
            this.inputs[i].widget = JSON.stringify(boolWidget, (key, value) => key === '_node' ? undefined : value);
